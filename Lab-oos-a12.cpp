...
class B : virtual public A {  // virtual to prevent multiples
   public:
    int _b;
    B(int a = 0xBA, int b = 0xB) : A(a), _b(b) {}
};

class C1 : public B {  // only non-virtual "C"-class, so C1::a and C1::b dont
                       // get summarized in the output for D2::print()
   public:
    C1(int a = 0xC1A, int b = 0xC1B) : B(a, b) {}
};

class C2 : virtual public B {  // virtual to prevent multiples
   public:
    C2(int a = 0xC2A, int b = 0xC2B) : B(a, b) {}
};

class C3 : virtual public B {  // virtual to prevent multiples
   public:
    ...
    D1(int c2_a = 0xD1C2A, int c3_a = 0xD1C3A, int c2_b = 0xD1C2B,
       int c3_b = 0xD1C3B)
        : A(0xD1A),
          C2(c2_a, c2_b),
          C3(c3_a, c3_b) {
    }  // add A(0xD1A) to the init list to get "D1A" in the output
    void print() {
        cout << "D1::print() ..." << endl;
    ...
    D2(int c1_a = 0xD2C1A, int c2_a = 0xD2C2A, int c3_a = 0xD2C3A,
       int c1_b = 0xD2C1B, int c2_b = 0xD2C2B, int c3_b = 0xD2C3B)
        : B(0x0, 0xD2B),
          ...
          C2(c2_a, c2_b),
          C3(c3_a, c3_b) {
    }  // same as in class D1, but the first parameter of B() is 0, because the
       // second var changes clas b´s valueF
